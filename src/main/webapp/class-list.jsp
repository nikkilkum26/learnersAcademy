...
<div class="body-container">
	<div class="row">
		...
		<div class="container">
			<h3 class="text-center">Subjects Master List</h3>
			<hr>
			<div >

				<a href="<%=request.getContextPath()%>/newclassroom" class="btn btn-success">Add
					New Subjects</a>
			</div>
			<br>
			<table class="table table-bordered">
				...
				<tbody>
					...
					<c:forEach var="c" items="${listClasses}">

						<tr>
							<td><c:out value="${c.id}" /></td>
							<td><c:out value="${c.name}" /></td>
						<td><c:out value="${c.teacher}" /></td>
 						<td><c:out value="${c.duration}" /></td> 
 						 <td><c:out value="${c.courses}" /></td> 
 						
						  <td><a href="editclassroom?id=<c:out value='${c.id}' />">Edit</a>
								&nbsp;&nbsp;&nbsp;&nbsp; <a
								href="deleteclassroom?id=<c:out value='${c.id}' />">Delete</a></td>  
						</tr>
					</c:forEach>
